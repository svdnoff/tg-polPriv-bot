...
ADMIN_IDS = [1014380197, 866973179]  # админы
BACKUP_ADMIN_ID = 866973179  # ID для отправки бэкапов
...
def backup_db(context=None):
    """Сохраняем резерв и отправляем администратору"""
    shutil.copyfile(DB_FILE, "tickets_backup.db")
    if context:
        try:
...
# ---------------- Магазины ----------------
SHOPS = {
    -1003450185997: {"address": "📍 Майкоп, Строителей 8Б", "work_time": "🕒 10:00–19:00", "max_link": "https://max.ru/join/IMHKjeOxfKJFcRQTQVrhlCGvLx-qOzAUiTpxCussSr0"},
    -1003777692701: {"address": "📍 Майкоп, Депутатская 16Б", "work_time": "🕒 10:00–19:00", "max_link": "https://max.ru/join/WZ8T-qgVdTK7He20c2UAvDcawKYbedKxKFmKVZbWovo"},
    -1003840431977: {"address": "📍 Лабинск, Победы 161", "work_time": "🕒 09:00–18:00", "max_link": "https://max.ru/join/caMNU_JQa9Q1-UlwqS1r6G9AECURkQn0ARdLGtM25wI"}
}
...
async def today(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for chat_id, name in REVIEW_CHATS.items():
        cursor.execute("SELECT number, user_id FROM tickets WHERE chat_id=? AND date=? ORDER BY number", (str(chat_id), today_date))
...
async def handle_review(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    cursor.execute("SELECT 1 FROM tickets WHERE chat_id=? AND user_id=? AND date=?", (str(chat_id), user_id, today_date))
    if cursor.fetchone():
        return
    ...
    cursor.execute("INSERT INTO tickets (chat_id, user_id, number, date, link) VALUES (?, ?, ?, ?, ?)",
                   (str(chat_id), user_id, number, today_date, link))
    conn.commit()
    backup_db(context)  # <-- бекап после вставки
...
async def stat(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = "📊 Статистика\n\n"
    for chat_id, name in REVIEW_CHATS.items():
        cursor.execute("SELECT COUNT(*) FROM tickets WHERE chat_id=? AND date=?", (str(chat_id), today_date))
...
async def reset_confirm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "reset_yes":
        ...
        conn.commit()
        backup_db(context)  # <-- бекап после сброса
